...
class CVCalibrationDialog(QDialog):
    ...
    def __init__(self, yaml_path, img_queue):
        super().__init__()
        ...
        calibrate_group = self.create_calibration_section()
    ...
    def create_calibration_section(self):
        ...
        self.calibrate_path_line = QLineEdit(self.settings.get("checkerboard", {}).get("calibration_path", ""))
        self.calibrate_path_line.setReadOnly(True)
        self.calibrate_path_line.setStyleSheet("""
            QLineEdit {
                background-color: #f8f8f2;
                color: #272822;
                padding: 5px;
        ...
        """)
        browse_calib_button = QPushButton("Browse Folder")
        browse_calib_button.clicked.connect(self.browse_calibrate_folder)
        calib_layout.addWidget(self.calibrate_path_line)
        calib_layout.addWidget(browse_calib_button)
        vbox.addLayout(calib_layout)

        # Calibrate button
        calibrate_button = QPushButton("Calibrate")
        calibrate_button.clicked.connect(lambda: self.calibrate(self.calibrate_path_line.text()))
    ...
    def browse_calibrate_folder(self):
        folder_path = QFileDialog.getExistingDirectory(self, "Select Calibration Folder")
        if folder_path:
            self.calibrate_path_line.setText(folder_path)
            self.calibrate_path_line.setToolTip(folder_path)
            if "checkerboard" not in self.settings:
                self.settings["checkerboard"] = {}
            self.settings["checkerboard"]["calibration_path"] = folder_path
    ...
    def capture_image(self):
        if not self.img_queue.empty():
            ...
            dummy_img = self.img_queue.get()
            cv2.imwrite(f"{self.settings['checkerboard']['calibration_path']}-{self.capture_count}.jpg", dummy_img)
            #dummy_img = (np.random.rand(480, 640) * 255).astype(np.uint8)
            height, width, _ = dummy_img.shape
    ...
    def calibrate(self,calib_path):
        ...
        calib_path = self.settings['checkerboard']['calibration_path']
        square_size = self.settings['checkerboard']['square_size']
        h = self.settings['checkerboard']['height']
        w = self.settings['checkerboard']['width']
        checkerboard_calib = CameraCalibration(calib_path,h,w)
        print("Calibrating Checkerboard...")
        mtx,dist,rvecs,tvecs = checkerboard_calib.checkerboard(square_size)
        # Create npy folder inside calibration path
        features_folder = os.path.join(calib_path, "features")
        os.makedirs(features_folder, exist_ok=True)

        # Save files
        np.save(os.path.join(features_folder, "mtx.npy"), mtx)
        np.save(os.path.join(features_folder, "dist.npy"), dist)
        np.save(os.path.join(features_folder, "rvecs.npy"), rvecs)
        np.save(os.path.join(features_folder, "tvecs.npy"), tvecs)

        print(f"[Saved] Calibration files saved to: {features_folder}")
        print("[Success] Camera Calibrated with known Camera Matrix, Distortion Coeff, Rotation, and Translation Vectors")
